...
module Prims : PRIMS = struct
  ...
  let our_prims = 
    let parts = [
      ...
      (* make_cons *)
      "MAKE_PAIR(rax, rsi ,rdi)", make_binary, "make_cons"; 

      (* set_car *)
      "mov qword [rsi+TYPE_SIZE], rdi", make_binary, "set_car"; 
      
      (* set_cdr *)
      "mov qword [rsi+TYPE_SIZE+WORD_SIZE], rdi", make_binary, "set_cdr";
      ...
      apply_done_fix_stack:
        ...
        mov rax, qword[rax]
        jmp rax
      ", make_routine, "apply"] in
    String.concat "\n\n" (List.map (fun (a, b, c) -> (b c a)) parts);;
  ...
  let procs = String.concat "\n\n" [type_queries; numeric_ops; misc_ops; our_prims];;
end;;
